...
- Replaced the old git-only `Scripts\release.ps1` helper with a routine publish/push orchestrator plus `Scripts\release.bat`; it builds, publishes, creates a portable zip, and commits/pushes tracked changes. GitHub tag/release creation and asset upload are opt-in through `Scripts\release.ps1 -CreateGitHubRelease` for major app milestones only.
