...
- Replaced the old git-only `Scripts\release.ps1` / `Scripts\release.bat` helper with `Scripts\GitHub Sync.ps1` / `Scripts\GitHub Sync.bat`; the routine sync builds, publishes, refreshes `E:\GoblinFarmer` from the newly published output while preserving runtime folders, creates a portable zip, and commits/pushes tracked changes. GitHub tag/release creation and asset upload are opt-in through `Scripts\GitHub Sync.ps1 -CreateGitHubRelease` for major app milestones only.
