...
- Latest utility cleanup: tracked `Scripts` now exposes only `Create Debug Package.bat` and `Create Project Brain.bat` as user-facing batch launchers. Required backing scripts are `create-debug-package.ps1` and `create-project-brain.ps1`; `debug-analysis-tools.ps1` remains because the debug package script directly loads it. Retired/manual helpers were moved to `Docs\ScriptArchive\2026-06-09` for reference. `create-project-brain.ps1 -StorageBreakdownOnly` writes `Reports\Storage_Breakdown.md` without deleting files.
...
- Current tracked `Scripts` policy: only two user-facing batch files are active: `Scripts\Create Debug Package.bat` and `Scripts\Create Project Brain.bat`.
...
- What changed: active script surface was simplified, obsolete/manual scripts were archived, `.gitignore`, README, Project Brain docs, release checklist, installer messages, project publish validation, and script-policy tests were updated, and the Project Brain PowerShell script gained storage-report mode.
- Tested: `Scripts\Create Debug Package.bat` created `DebugPackages\GoblinFarmer_Debug_20260608_231533.zip` and performed expected retention by deleting the prior oldest matching ZIP to keep 20 packages; `Scripts\Create Project Brain.bat` created `ProjectBrain\GoblinFarmer_ProjectBrain_20260608_231551.zip`; `create-project-brain.ps1 -StorageBreakdownOnly` created `Reports\Storage_Breakdown.md`; `dotnet build GoblinFarmer.csproj` passed; `dotnet build .\GoblinFarmer.csproj -p:UseSharedCompilation=false` passed; `dotnet run --project .\Tests\GoblinFarmer.Tests\GoblinFarmer.Tests.csproj -p:UseSharedCompilation=false` passed; `git diff --check` passed with line-ending normalization warnings only.
...
- Next recommended task: review `Reports\Storage_Breakdown.md`, decide which generated/debug artifacts can be trimmed, and then do a separate explicit cleanup pass without touching source scripts.
