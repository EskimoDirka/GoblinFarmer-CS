...
## Recently Fixed
- Sanitized current repo privacy surface before the next GitHub sync: tracked config no longer carries local Diablo/Battle.net executable paths, helper scripts default to the current user's local app-data install folder instead of a machine-specific drive, docs no longer record local install/repo paths, and executable handoff files remain visible for intentional GitHub sync.
...
- Added `Scripts\create-debug-package.bat` to build/publish content, publish validation, and Inno preflight validation. The installed app folder now contains both `Scripts\create-debug-package.ps1` and `Scripts\create-debug-package.bat`.
...
- Replaced the old git-only `Scripts\release.ps1` / `Scripts\release.bat` helper with `Scripts\GitHub Sync.ps1` / `Scripts\GitHub Sync.bat`; the routine sync builds, publishes, refreshes the local user app folder from the newly published output while preserving runtime folders, copies the newly published executable to tracked `GitHub Upload\GoblinFarmer.exe`, creates a portable zip, and commits/pushes tracked changes. GitHub tag/release creation and asset upload are opt-in through `Scripts\GitHub Sync.ps1 -CreateGitHubRelease` for major app milestones only.
