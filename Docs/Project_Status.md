...
- Existing goblin evidence candidate hooks (`JournalCandidate`, `MinimapCandidate`) route through the same guarded counter. Journal parsing treats killed and escaped goblin lines as encounter evidence, and normalizes `Gelatinous Spawn` journal kills to `Gelatinous Sire`.
...
- `Sessions\Session_YYYYMMDD_HHMMSS.md` is exported at app shutdown with app version/build mode, session start/end/duration, reliability counters, combat active time, Goblin Tracker count, found-record count, counted-area count, active time/GPH, latest log/debug package/screenshot paths, latest failure type, and last known issue.
...
- GoblinEvidence retention runs at startup and after manual Ctrl+Shift+G calibration captures, deleting only files under `Debug\GoblinEvidence` beyond `Debug.GoblinEvidenceRetentionCount` newest files. The default is `250`; zero or negative disables cleanup.
...
- Debug package console output and `debug-package-manifest.txt` report total/success/failure/normal screenshot counts, excluded stale screenshot count, excluded GoblinEvidence full-image count, current GoblinEvidence source count/size when present, package size, package folder totals, session start, session duration, and Goblin Tracker metadata from `session-info.txt`.
