...
- `Config\AppSettings.json` now also stores Diablo III and Battle.net executable paths, the relative Images root, scan-region cache path, launch timings, image-recognition thresholds, and the selected combat profile under `User.CombatProfile`. Missing executable paths trigger auto-discovery and first-run setup instead of relying on source-machine install paths.
...
- Added `User.CombatProfile` to `Config\AppSettings.json`, restored the selected character radio button on startup, and saved profile changes when Monk, Demon Hunter, or Witch Doctor is selected.
