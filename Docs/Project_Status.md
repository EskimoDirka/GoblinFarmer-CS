...
- Reviewed `GoblinFarmer_Debug_20260603_154213.zip`: after Southern Highlands, a fresh hotkey scan falsely matched Northern Highlands at confidence `0.816`; the already-at-queued shortcut advanced route state and sent the hotkey to The Weeping Hollow. Tightened the shortcut so matching the normal queued next step preserves route order and keeps the target on the queued destination.
- Reviewed `GoblinFarmer_Debug_20260603_155242.zip`: the Southern Highlands hotkey flow now logs `AlreadyAtQueuedDestinationAdvanceSkipped`, preserves Northern Highlands as the queued target, confirms Northern Highlands before Weeping Hollow, and later logs `Automation number hotkey ignored by self-injection guard` for an app-generated `1`. The remaining oddities were Battle.net launch diagnostic noise and one Start Game verification retry that recovered.
...
- Refreshed the self-contained publish output and `E:\GoblinFarmer` user executable after the debug/options UI cleanup so the installed/user app no longer runs stale UI bits.
...
- Published the fixed Release package with `Scripts\publish-release.ps1 -SkipInstaller`, refreshed `E:\GoblinFarmer` while preserving its `Config`, logs, packages, and screenshots, and validated the installed executable directly. `E:\GoblinFarmer\GoblinFarmer.exe` now starts compact at `934x649`, shows no Skill 1 teleport-protection checkbox, expands to `1366x649` when Debug Mode is checked, auto-checks Keep Debug Screenshots, and shrinks back to `934x649` with Keep Debug Screenshots hidden when Debug Mode is unchecked.
- Rebuilt the Inno Setup installer from the fixed publish folder using `Scripts\publish-release.ps1`, then installed `artifacts\installer\GoblinFarmerSetup-1.3.0.exe` silently to `E:\GoblinFarmer`. The installed executable hash matched the fixed publish executable, and UI Automation confirmed the same release/debug UI behavior from the installer-installed app.
- Built, published, compiled, and installed after changing hotkey blocked-location notifications to use the fresh current-location PNG match, including Caldeum sublocations such as Gates of Caldeum or Sewers of Caldeum, while route button state remains grouped. `Scripts\create-debug-package.ps1` is now copied to publish output, enforced by publish/Inno validation, included by the installer, and present at `E:\GoblinFarmer\Scripts\create-debug-package.ps1`.
