...
- The manual Goblin Tracker hotkey is global physical `X`; it resolves the current area from a fresh current-location scan when available, falls back to the cached confirmed location, and counts only once per resolved area key per game. If no area can be resolved, the hotkey falls back to the existing count behavior and logs the unresolved area.
- Goblin duplicate protection uses an in-memory per-game counted-area guard. Same-area duplicates are suppressed with `AreaAlreadyCounted`, different resolved areas count separately, and the guard clears on new-game creation and `Reset Stats`.
...
- Failure milestones capture paired Diablo/App screenshots using matching timestamps and action names. Success screenshots are controlled by `Debug.EnableSuccessScreenshots`, default false.
...
- Excludes `Screenshots\Success` by default; use `-IncludeSuccessScreenshots -MaxSuccessScreenshots N` only if a debug session explicitly needs success evidence in the zip.
- When `Debug.EnableSuccessScreenshots=true` and current-session success screenshots exist, the manifest reports the available success screenshot count and total size without automatically including them.
