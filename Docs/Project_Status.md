...
## Current Focus
v1.4.0 foundation work has started with the manual Goblin Tracker and GPH counter. The tracker is intentionally manual-only: physical `X` increments the current-session goblin count while GoblinFarmer is running, GPH uses resettable tracker active combat time, and no image recognition, OCR, minimap detection, or automatic goblin detection has been added. The first Debug Manager / Diagnostics Manager pass remains implemented through `DebugManager` and `DiagnosticsSessionState`, centralizing debug profile behavior, screenshot/session artifact paths, diagnostic throttling, reliability counters, tracker stats, and active-time snapshots. Reliability work around Battle.net launch, Diablo launch detection, Start Game stability/recovery, Make New Game workflow protection, teleport routing accuracy, combat safety, town workflows, and synchronized app/installer versioning remains stable. Remaining work is live validation and future hardening, not blocking implementation.
...
- The manual Goblin Tracker hotkey is global physical `X`; it increments the session goblin count once per press while GoblinFarmer is running and does not require Diablo focus or active combat.
...
- `Sessions\Session_YYYYMMDD_HHMMSS.md` is exported at app shutdown with app version/build mode, session start/end/duration, reliability counters, combat active time, Goblin Tracker count/active time/GPH, latest log/debug package/screenshot paths, latest failure type, and last known issue.
...
- Debug package console output and `debug-package-manifest.txt` report total/success/failure/normal screenshot counts, excluded stale screenshot count, package size, session start, session duration, and Goblin Tracker metadata from `session-info.txt`.
