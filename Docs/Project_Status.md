...
- Goblin Replay Fixture Runner Phase 2D is implemented as explicit/on-demand harness support. Live scans still use `CopyFromScreen` by default, saved Journal/Minimap PNG fixtures can be fed through the shared frame/template matching path, and real saved encounter/manual capture folders can be loaded by the harness for stale-location regression coverage without normal startup, VS Debug startup, scanner, route, combat, town, or debug package workflows invoking replay.
...
- Next Goblin Replay work should add an explicit harness command or developer script for selecting real capture folders, still without auto-running during app startup or changing production count behavior.
