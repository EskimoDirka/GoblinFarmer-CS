...
- Replaced the old git-only `Scripts\release.ps1` helper with a full local release orchestrator plus `Scripts\release.bat`; it builds, publishes, creates a portable zip, commits/pushes tracked changes, tags the version, and uploads installer/zip assets through GitHub CLI, including standard-location `gh.exe` discovery when GitHub CLI is installed but not on `PATH`.
