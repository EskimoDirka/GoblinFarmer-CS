...
- The debug package batch runs Goblin Replay before packaging when `Debug\GoblinEvidence` is available, and the ZIP includes `goblin-tracker-review.html`, `goblin-tracker-summary.txt`, `goblin-tracker-next-tests.txt` when available, replay artifacts, decision bundles, latest log, route summaries, screenshots, GoblinEvidence, and `session-info.txt` when available
...
- The package batch auto-discovers the freshest `Debug\GoblinEvidence` folder, runs the Goblin Replay CLI before ZIP creation unless `-SkipGoblinReplay` is passed, and records replay attempted/succeeded/input/runner/config details in the manifest
...
- Goblin replay package artifacts are generated by the debug package batch when evidence is available, so the normal handoff is one ZIP instead of a separate replay package
