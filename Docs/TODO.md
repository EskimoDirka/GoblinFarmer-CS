...
- [x] Review `GoblinFarmer_Debug_20260608_213351.zip`: Leoric's Hunting Grounds Treasure Goblin counted from Minimap with the latency fix intact. The later Eastern Channel Level 2 miss did not show fresh countable evidence: Minimap stayed below threshold and Journal matched old Leoric evidence that was correctly suppressed after candidate-specific area resolution selected Eastern Channel Level 2. Added route-context diagnostics to scan/result/freshness logs so future packages can distinguish raw channel location from the grouped `Ancient Waterway` route button context.
...
- [x] Live-validate the `500ms` scanner interval and strong-Minimap-over-pending-Journal fix improve notification latency without lowering evidence thresholds or increasing false positives.
...
- [ ] VS Debug: open the `Sim Count` dropdown and confirm `Current Area` is first, remaining areas are alphabetized, and route/count/block validation areas are available, including Eastern/Western Channel levels, Cave/Caverns levels, Battlefields/Rakkis, PF1/PF2, Stinging Winds, and blocked areas such as New Tristram/WhimsyDale/Ancient Waterway.
...
- [ ] Re-check Eastern Channel Level 2 after the route-context diagnostic pass if another fresh goblin misses. Expected logs should show whether `routeRawArea`/`GoblinTracker: Area resolved` stayed on Eastern Channel Level 2, whether Minimap crossed threshold, and whether Journal evidence was fresh or old-area suppressed.
...
- [x] Live-validate notification latency after the strong-Minimap-over-pending-Journal fix. Latest live context reports the latency fix stable with no regression.
