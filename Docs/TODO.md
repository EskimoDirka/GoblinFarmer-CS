...
* [ ] Validate Witch Doctor mouse-wheel/cursor-click mode: select Witch Doctor, toggle combat on, confirm logs show `combatClass=witch_doctor`, `combatInputMode=MouseWheelScroll`, repeated `WitchDoctorMouseWheelScrollSent`, key loop order `2,3,1`, `heldLeftMode=false`, `heldRightMode=false`, discrete `WitchDoctorCursorChangeRightClickSent` only after Diablo cursor changes, `WitchDoctorCursorChangeRightClickSkipped` over no-click UI, and no Witch Doctor held-left or held-right release.
...
* [x] Replace Witch Doctor held/channel behavior with mouse-wheel scrolling plus discrete cursor-change right-click pulses while preserving the `2,3,1` key loop and no-click safety logs.
