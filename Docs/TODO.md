...
- [ ] Live-validate the reliability gate during normal play: Engaged-only Journal evidence should not count by itself; fresh Journal Killed or Minimap evidence should still count promptly and log `evidenceReliability=JournalKilledConfirmed` or `evidenceReliability=MinimapConfirmed`.
- [ ] Live-validate the notification splash is click-through and no longer blocks route/teleport mouse clicks.
