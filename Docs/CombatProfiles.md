...
- Witch Doctor: starts the key loop, mouse-wheel loop, and cursor-change left-click loop together; uses mouse wheel scrolling as the repeated mouse action; keeps the `2, 3, 1` key loop running; and sends only discrete cursor-change left-click pulses. It does not use held-left/channel mouse mode or held-right mode.
