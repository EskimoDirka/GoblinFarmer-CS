...
- Debug Mode with diagnostic panes, screenshot controls, route state inspection, Goblin Replay export, and one batch-driven debug package workflow for VS Debug and Release.
...
For both VS Debug and Release troubleshooting, use the single ZIP export path: `Scripts\Create Debug Package.bat`, which launches `Scripts\create-debug-package.ps1`. The batch script self-discovers the runtime/package roots, runs Goblin Replay first when GoblinEvidence is available, writes the ZIP under `DebugPackages`, and includes the latest logs, manifests, route summaries, Goblin Tracker summaries, Next Tests metadata when available, screenshots, GoblinEvidence, encounter captures, decision bundles, and replay artifacts. Closing the VS Debug form is intentionally quiet and does not generate packages or loose review files.
...
- Root Goblin Tracker review index and summary, Next Tests metadata, encounter captures, decision bundles, and replay reports generated by the package batch when GoblinEvidence is available.
...
Generate a debug package by double-clicking `Scripts\Create Debug Package.bat`. The batch runs Goblin Replay first when it finds `Debug\GoblinEvidence`, then creates one ZIP with both live evidence and replay results. From the project root:
