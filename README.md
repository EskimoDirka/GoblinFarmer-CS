...
- Debug Mode with diagnostic panes, screenshot controls, route state inspection, VS Debug OBS recording status, and one batch-driven debug package workflow for VS Debug and Release.
...
Debug Package Replay is also explicit/on-demand. Debug Mode salvage and gem-stash scans write structured replay logs under `Debug\ReplayLogs`; they no longer create automatic inventory replay screenshot folders. The normal debug package script attempts to find the newest OBS video under `Video Clip Review`, align high-value log events to video offsets, extract selected frames into `ReviewEvidence\manifest.json`, `ReviewEvidence\issue.md`, and `ReviewEvidence\frames`, and print the selected clip path plus duration, size, and modified time in the final PowerShell summary. Manual timestamps and notes can still be supplied when the automatic alignment misses something. Replay reads the package ZIP/folder without clicking Diablo:
...
- Structured inventory replay JSONL events under `Debug\ReplayLogs`, plus package-level `ReviewEvidence` for selected OBS frames/manual crops.
...
Generate a debug package by double-clicking `Scripts\Create Debug Package.bat`. The batch creates one ZIP from the existing live runtime evidence and attempts to add log-aligned frames from the newest OBS recording. From the project root:
