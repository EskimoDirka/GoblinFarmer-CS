...
- Make New Game, Exit Game, repair, salvage, gem stashing, and town workflow helpers. Salvage uses a fast cached inventory scan with bounded recovery rescans when accepted leftovers remain, can run a gated blue/yellow bulk category pass when enabled, skips normal gems as non-salvageable including stack-count, tooltip-covered live gem-stack, and amber body gem shapes, then clicks one-column item anchors with legal one- or two-row footprints and confirmation-aware handling for set and legendary-style targets. Auto gem stashing is separate under `Images\Gems`, defaults off in release config, defaults on in VS Debug for validation, skips safely when assets are missing, waits after the stash-coordinate travel click before opening stash, and right-clicks accepted gem-template or gem-color matches.
...
Debug Package Replay is also explicit/on-demand. Debug Mode salvage and gem-stash scans write structured replay logs under `Debug\ReplayLogs`; they no longer create automatic inventory replay screenshot folders. The normal debug package script attempts to find the newest NVIDIA/attached video under `Video Clip Review`, align high-value log events to video offsets, extract selected frames into `ReviewEvidence\manifest.json`, `ReviewEvidence\issue.md`, and `ReviewEvidence\frames`, and print the selected clip path plus duration, size, modified time, and review notes URL in the final PowerShell summary. Manual timestamps, notes, and the shared Google Sheet notes URL can still be supplied when the automatic alignment misses something. Replay reads the package ZIP/folder without clicking Diablo:

```powershell
powershell -ExecutionPolicy Bypass -File .\Scripts\create-debug-package.ps1 -ReviewVideoPath "D:\D3\Projects\GoblinFarmer\GoblinFarmer\GoblinFarmer\Video Clip Review\Desktop.mkv" -ReviewTimestamp "00:02:14.500=gem-stash-leftover" -ReviewNotesPath ".\Debug Evidence\issue.md"
dotnet run --project .\Tests\GoblinFarmer.Tests\GoblinFarmer.Tests.csproj -- --debug-package-replay "D:\Path\To\DebugPackages\GoblinFarmer_Debug_...zip"
```
...
Debug and release artifacts keep evidence useful but bounded by default. VS Debug troubleshooting artifacts and release/debug-mode artifacts use a 7-day age-retention window, and debug package ZIP count retention keeps the newest 20 matching packages by default. Success screenshots are excluded from release/export packages unless explicitly requested, failure screenshot groups and current-session `debug-screenshots` are capped to recent samples, and Goblin Evidence crops/event screenshots use their own count and package limits. Normal scanner events skip redundant root `GoblinEvidence_*` full/event images by default, DecisionBundles keep replay-ready Journal/Minimap crops and metadata without copying full evidence images by default, and debug ZIPs exclude old DecisionBundle full images plus Encounter/ManualCapture fullscreen images unless explicitly opted in. Runtime salvage/gem-stash replay evidence is log-first under `Debug\ReplayLogs`; reviewed video frames are included only when requested under `ReviewEvidence`, and full videos plus legacy `Debug\InventoryReplay` screenshot folders are excluded by default. Successful Battle.net app-click launches do not create false failure screenshots for manual-play suspicion or brief post-launch Battle.net visibility. The package manifest reports included and excluded screenshot counts and sizes.

For both VS Debug and Release troubleshooting, use the single ZIP export path: `Scripts\Create Debug Package.bat`, which launches `Scripts\create-debug-package.ps1` and passes through optional PowerShell arguments. The batch script self-discovers the runtime/package roots, writes the ZIP under `DebugPackages`, attempts automatic review-video/log evidence extraction, and includes the latest logs, manifests, route summaries, Goblin Tracker summaries, bounded screenshots, GoblinEvidence, replay-ready encounter captures, decision bundles, observation diagnostics, structured replay logs, reviewed video evidence when a matching clip/log event is found, and root analysis files: `debug-package-analysis.txt`, `goblin-tracker-timeline.md`, and `goblin-evidence-health.txt`. Closing the VS Debug form is intentionally quiet and does not generate packages or loose review files.
...
- Structured inventory replay JSONL events under `Debug\ReplayLogs`, plus package-level `ReviewEvidence` for selected review-video frames/manual crops.
...
- Age-based debug artifact retention: 7 days for VS Debug troubleshooting folders and release/debug-mode artifacts, plus count-based debug package retention defaulting to the newest 20 ZIPs, GoblinEvidence retention under `Debug\GoblinEvidence` defaulting to the newest 250 files, replay logs under `Debug\ReplayLogs`, and bounded ObservationDiagnostics/review-evidence frame samples in debug packages.
...
Generate a debug package by double-clicking `Scripts\Create Debug Package.bat`. The batch creates one ZIP from the existing live runtime evidence and attempts to add log-aligned frames from the newest NVIDIA/attached recording under `Video Clip Review`. From the project root:

```powershell
powershell -ExecutionPolicy Bypass -File .\Scripts\create-debug-package.ps1
powershell -ExecutionPolicy Bypass -File .\Scripts\create-debug-package.ps1 -ReviewVideoPath ".\Video Clip Review\Desktop.mkv" -ReviewTimestamp "00:02:14.500=gem-stash-leftover" -ReviewNotesPath ".\Debug Evidence\issue.md"
```
...
GoblinFarmer v1.5 is focused on town workflow reliability: faster salvage, confirmation-safe set/legendary handling, log-first replay diagnostics, and gated normal-gem auto-stashing.
...
- Added reviewed video evidence support in debug packages without including full video files or automatic replay screenshot folders by default.
