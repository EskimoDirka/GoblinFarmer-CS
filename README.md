...
The latest reviewed package, `GoblinFarmer_Debug_20260602_220408.zip`, validated Teleport Next already-at-queued-destination advancement and manual-button bypass behavior. It also showed Bounty Complete could be detected while combat was active but Escape was skipped until combat stopped, so the title template was refreshed and detected bounty popups now send automation-safe injected Escape during combat. The current recommended validation target is a focused run for WhimsyDale blocking, Bounty Complete combat-close, Start Game consistency, and config toggles. See [Docs/Project_Status.md](Docs/Project_Status.md) for the exact current focus and latest known issues.
