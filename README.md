...
- Debug Mode with diagnostic panes, screenshot controls, route state inspection, VS Debug review files, and release/export debug package generation.
...
Automation Observation Mode watches Journal and Minimap goblin candidates and records what the automation would have counted. It discovers per-goblin templates from `Images\Goblin Evidence` using names such as `<Goblin Type> Engaged Journal.png`, `<Goblin Type> Killed Journal.png`, optional cropped journal names like `<Goblin Type> Engaged.png`, prefix names like `Killed <Goblin Type> Journal.png`, and tight `<Goblin Type> Minimap.png` icon crops, then reports the matched goblin type when possible. The observation scanner is enabled by default through `GoblinTracker.EnableObservationMode=true` and runs while Diablo is active/focused, even outside combat. Automatic counting is controlled separately by `GoblinTracker.EnableAutomaticCounting=false`, which defaults off. Automatic counting can run only when both Observation Mode and Automatic Counting are enabled; VS Debug builds expose both settings inside Settings as `Observation Mode` and `Auto Goblin Count` checkboxes and persist them to the project AppSettings file. VS Debug also exposes `Test Count Override`, `Enable Goblin Decision Trace`, `Review Files`, and a `Next Tests` diagnostics tab with a route-ordered automatic-count readiness checklist. Each `Next Tests` row is a checkbox: unchecked means not tested yet, and checked means the tester completed that scenario during the current run. `Review Files` runs Goblin replay against the active runtime `Debug\GoblinEvidence` folder and refreshes loose review artifacts under `Debug\GoblinReplayReview\Latest` without prompts or a ZIP; release/export troubleshooting can still use the existing debug package ZIP workflow. The test override is effective only in VS Debug and only bypasses the fresh-evidence requirement for manual `X`, leaving blocked areas, duplicate suppression, and PF1/PF2/Stinging Winds limits intact. The offline replay engine still accepts saved evidence folders or debug package ZIPs, extracts ZIPs into a temporary workspace, runs saved Journal/Minimap images through the same template matching path without Diablo, writes a dry-run `Logs\GoblinReplay_yyyyMMdd_HHmmss.log`, and writes a companion HTML report with thumbnails, best match, top candidate ranking, area inference, final decision, and comparison against the previous replay. Replay does not change GoblinCount, GPH, active time, found records, or route state. Live automatic-count decisions emit `GoblinDecisionTrace` with a correlation id and save a matching decision bundle under `Debug\GoblinEvidence\DecisionBundles` when an evidence image is available. Journal evidence is primary when both Journal and Minimap match in the same normal scan, and Engaged journal lines remain the preferred current-encounter anchor, but fresh same-area Killed journal lines can also count as current evidence when Engaged misses. Treasure Goblin and Odious Collector minimap matches get a targeted yellow-vs-green color disambiguation pass before the observed goblin type is displayed or auto-counted, and Gilded Baron / Malevolent Tormentor minimap matches now include yellow-vs-orange diagnostics and overrides when the patch is clearly dominant. Non-ambiguous minimap auto-counts use a `0.85` confidence gate, while the Gilded Baron / Malevolent Tormentor pair keeps the stricter `0.90` gate. Manual `X` refreshes can also use fresh same-area Killed journal evidence when Engaged templates miss, while stale visible Engaged or Killed journal rows are suppressed by first-seen freshness and coarse row identity. Observation Mode uses the same safer area resolution, blocked-area, duplicate, and two-count exception rules as manual `X`; when Automatic Counting is disabled, observations do not change GoblinCount, GPH, active time, found records, or counted-area slots, but their first-seen evidence signatures are still remembered so toggling Automatic Counting on cannot count already-visible evidence as new. When Automatic Counting is explicitly enabled, the app requires fresh eligible evidence first seen after the gate was armed, scopes evidence signatures by resolved area so separate levels/subregions can count independently, suppresses already-used same-area evidence signatures, suppresses the same counted Journal evidence row if it reappears in another area, requires Engaged and Killed journal evidence to be first seen in the current resolved area, suppresses lower-confidence minimap-only evidence until Journal or a stronger minimap match appears, and shows a no-activate notification when an automatic count increments GoblinCount. Last Observation stays on the last real goblin until a new observation/count, reset/new-game cleanup, or confirmed current-area change replaces it; no-candidate scans and route-state updates clear a persisted observation when the current confirmed area no longer matches the displayed observation area. Accepted manual `X` notifications reuse a recent same-area observed goblin type when available, otherwise run a Minimap-first current observation refresh before suppressing if no fresh evidence exists. Debug logs include Last Observation update/clear/preserve state, Observation Mode configuration, scanner start/stop with a 750ms interval, scan attempts/skips, candidate checks with match points, calibrated Journal/Minimap scan regions, journal freshness diagnostics, minimap color diagnostics, automatic-count accepted/suppressed decisions when enabled, structured `GoblinDecisionTrace` lines, route-button click receipt/queue/execution diagnostics, suppressed route-button queued feedback, and throttled diagnostic crop paths.
...
Debug and release artifacts keep evidence useful but bounded by default. VS Debug troubleshooting artifacts are age-retained for 24 hours, while release/debug-mode artifacts use a 7-day age-retention window. Success screenshots are excluded from release/export packages unless explicitly requested, failure screenshot groups and current-session `debug-screenshots` are capped to recent samples, and Goblin Evidence crops/event screenshots use their own count and package limits. Successful Battle.net app-click launches do not create false failure screenshots for manual-play suspicion or brief post-launch Battle.net visibility. The package manifest reports included and excluded screenshot counts and sizes.

In VS Debug, the `Review Files` button is one-click: it captures the current route-ordered `Next Tests` checkbox state automatically, runs Goblin replay, and refreshes loose files under `Debug\GoblinReplayReview\Latest`. That folder includes `goblin-tracker-review.html`, `goblin-tracker-summary.txt`, `goblin-tracker-next-tests.txt`, the replay log, HTML report, grouped summary, changed-decision summary, and per-observation decision bundles when available. There are no freeform scenario input prompts and no ZIP is created for the default VS Debug troubleshooting workflow. Release/export troubleshooting can still create a debug package ZIP from the script or release package button when needed. A terminal helper is also available at `Scripts\replay-goblin-evidence.ps1` to summarize the latest package artifacts from `DebugPackages`.
...
- Route and workflow summaries in generated release/export debug packages.
- Root Goblin Tracker review index, grouped decision summary, changed-decision report, Next Tests metadata, and replay decision bundles.
...
- Age-based debug artifact retention: 24 hours for VS Debug troubleshooting folders and 7 days for release/debug-mode artifacts, plus count-based GoblinEvidence retention under `Debug\GoblinEvidence` defaulting to the newest 250 files and bounded ObservationDiagnostics crop samples in debug packages.
