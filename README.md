...
Goblin Tracker debug packages now include a root `goblin-tracker-review.html` index and `goblin-tracker-summary.txt` so package review can start from one place. In VS Debug, the `Create Debug Package` button is one-click: it captures the current route-ordered `Next Tests` checkbox state automatically, runs Goblin replay, and packages the replay log, HTML report, grouped summary, changed-decision summary, per-observation decision bundles, and `goblin-tracker-next-tests.txt` when available. There are no freeform scenario input prompts for debug-package creation. A terminal helper is also available at `Scripts\replay-goblin-evidence.ps1` to summarize the latest package artifacts from `DebugPackages`.
...
- Root Goblin Tracker package review index, grouped decision summary, changed-decision report, Next Tests metadata, and replay decision bundles.
