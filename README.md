...
The Goblin Tracker counts goblins from fresh Minimap evidence or fresh Journal Killed evidence when both `GoblinTracker.EnableObservationMode=true` and `GoblinTracker.EnableAutomaticCounting=true`. Journal Engaged-only evidence is treated as diagnostic/pending confirmation and does not increment by itself. VS Debug builds expose these as `Observation Mode` and `Auto Goblin Count` checkboxes. Observation Mode can still run without automatic counting, in which case Last Observation updates but GoblinCount, GPH, found records, and area slots are unchanged.
