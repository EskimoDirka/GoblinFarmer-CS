...
- Goblin Replay against real capture folders is a developer-only explicit harness command: `dotnet run --project .\Tests\GoblinFarmer.Tests\GoblinFarmer.Tests.csproj -- --goblin-replay-captures --templates ".\Images\Goblin Evidence" "<CaptureFolder>"`. Keep it out of app startup, VS Debug startup, scanner, combat, route, town, and debug package workflows.
