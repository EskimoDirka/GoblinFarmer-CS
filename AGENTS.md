...
- Treat every user live-test run as a VS Debug run unless the user explicitly says otherwise. For Goblin Tracker evidence issues, ask the user to click `Scripts\Create Debug Package.bat` after the run, then review the newest ZIP in `DebugPackages`. The batch launcher delegates to `Scripts\create-debug-package.ps1` and is the single intentional review/export workflow for both VS Debug and Release. Inspect `debug-package-analysis.txt`, `goblin-tracker-timeline.md`, `goblin-evidence-health.txt`, `debug-package-manifest.txt`, `goblin-tracker-summary.txt`, `goblin-tracker-review.html`, `goblin-tracker-next-tests.txt` when present, latest logs, route summaries, screenshots, GoblinEvidence, encounter captures, decision bundles, and observation diagnostics. The optional helper scripts under `Scripts\` may analyze an existing package directly, but they must not replace the batch-created ZIP workflow. Form close is a quiet shutdown path and must not be used as a debug artifact generation trigger. Do not run a separate evidence-reprocessing workflow unless the user explicitly introduces a new one.
