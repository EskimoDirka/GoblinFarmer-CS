...
- Treat every user live-test run as a VS Debug run unless the user explicitly says otherwise. For Goblin Tracker evidence issues, use the automatically generated VS Debug loose review files produced when the form closes; the app records the `Next Tests` checkbox state, runs Goblin replay against `Debug\GoblinEvidence`, and refreshes `Debug\GoblinReplayReview\Latest` without prompting or creating a ZIP. Review `Debug\GoblinReplayReview\Latest\goblin-tracker-review.html`, `goblin-tracker-summary.txt`, and `goblin-tracker-next-tests.txt`, then compare `Logs\GoblinReplay\GoblinReplay_*` log/HTML/summary/changed files, encounter minimap/journal crops, and replay decision bundles with the live notes. ZIP debug package creation is reserved for release/export troubleshooting through `Scripts\Create Debug Package.bat` or `Scripts\create-debug-package.ps1`.
