...
* For latest-debug-package review prompts, treat the named/latest debug package and live-test notes as evidence before changing code. Review the newest ZIP in `DebugPackages` when provided or requested. The batch launcher `Scripts\Create Debug Package.bat` delegates to `Scripts\create-debug-package.ps1` and is the intentional ZIP export workflow for both VS Debug and Release. Inspect `debug-package-analysis.txt`, `goblin-tracker-timeline.md`, `goblin-evidence-health.txt`, `debug-package-manifest.txt`, `goblin-tracker-summary.txt`, `goblin-tracker-review.html`, latest logs, route summaries, screenshots, GoblinEvidence, encounter captures, decision bundles, and observation diagnostics. Archived optional helper scripts under `Docs\ScriptArchive\2026-06-09` may be referenced manually, but they must not replace the batch-created ZIP workflow. Form close is a quiet shutdown path and must not be used as a package/export trigger. Do not run a separate evidence-reprocessing workflow unless the user explicitly introduces a new one.
* For AI-assisted development handoff context, use `Scripts\Create Project Brain.bat`, which delegates to `Scripts\create-project-brain.ps1` and writes a docs-only timestamped ZIP under `ProjectBrain`. This package includes `AGENTS.md`, `README.md`, `Docs\Project_Status.md`, `Docs\ProjectBrain\*.md`, and selected small stable `Docs\*.md` reference files when present; it must not include runtime artifacts, screenshots, images, replay evidence, binaries, or existing ZIPs. For repository storage review, run `Scripts\create-project-brain.ps1 -StorageBreakdownOnly`; it writes `Reports\Storage_Breakdown.md` and must not delete files.
* For project storage cleanup, use `Scripts\Cleanup Project.bat` or `Scripts\cleanup-project.ps1` as a maintenance-only exception to the normal two package launchers. The cleanup script must run dry-run by default; real deletion requires `-Delete`. Optional runtime evidence cleanup requires `-RuntimeArtifacts`, old installer pruning requires `-PruneOldInstallers`, and debug package pruning uses `-DebugPackageRetention 10` by default. Do not run real cleanup on the repository unless the user explicitly requests deletion.
